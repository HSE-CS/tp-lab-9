...
void Shop::work(){
	...
	while (this->buyers.size() || this->cashBoxes.size()) {
		for (std::set<buyer*>::iterator b = this->buyers.begin(); b != this->buyers.end();) {
			if ((*b)->timeToPay <= time) {
				bool wasAdd{ false };
				mu.lock();
				for (std::set<std::pair<std::thread*, std::queue<buyer*>*>>::iterator cb = this->cashBoxes.begin();
					...
					if (!(*cb).second->size()) {
						...
						if ((*cb).second->size() < this->lim) {
							(*cb).second->push(*b);
							std::cout << "add buyer to " << (*cb).first->get_id() << std::endl;
							wasAdd = true;
						}
						cb++;
					}
				...
				if (!wasAdd) {
					...
					std::cout << "add the first buyer to " << newT->get_id() <<  std::endl;
		...
		std::cout << this->buyers.size() << ' ' << this->cashBoxes.size() << std::endl;
