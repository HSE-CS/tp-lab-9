  ...
  while (this->buyers.size() || this->cashBoxes.size()) {
    for (std::set<buyer*>::iterator b = this->buyers.begin();
      ...
      if ((*b)->timeToPay <= time) {
        bool wasAdd{false};
        mu.lock();
        for (std::set<std::pair<std::thread*, std::queue<buyer*>*>>::iterator
                 cb = this->cashBoxes.begin();
          ...
          if (!(*cb).second->size()) {
            ...
            if ((*cb).second->size() < this->lim) {
              (*cb).second->push(*b);
              std::cout << "add buyer to " << (*cb).first->get_id()
                        << std::endl;
              wasAdd = true;
            }
            cb++;
    ...
    for (std::set<std::pair<std::thread*, std::queue<buyer*>*>>::iterator cb =
             this->cashBoxes.begin();
         cb != this->cashBoxes.end();) {
      if (!(*cb).second->size()) {
        cb = this->cashBoxes.erase(cb);
      } else {
    ...
    std::cout << this->buyers.size() << ' ' << this->cashBoxes.size()
              << std::endl;
